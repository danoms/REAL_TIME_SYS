...
void low_task_func(void *arg) {
	...
	print_pri(&low_task, "low : taking mut A \n");
	// rt_sem_p(&semaphore, TM_INFINITE);
	rt_mutex_acquire(&mutA, TM_INFINITE);
	rt_task_set_priority(&low_task, H);

	print_pri(&high_task, "low : busy waiting\n");
	busy_wait_ms(3);
	print_pri(&low_task, "low : taking mut B\n");
	// rt_sem_p(&semaphore, TM_INFINITE);
	rt_mutex_acquire(&mutB, TM_INFINITE);

	print_pri(&low_task, "low : busy waiting\n");
	busy_wait_ms(3);

	print_pri(&low_task, "low : releasing mut A\n");
	// rt_sem_v(&semaphore);
	rt_mutex_release(&mutA);

	print_pri(&low_task, "low : releasing mut B\n");
	// rt_sem_v(&semaphore);
	rt_mutex_release(&mutB);

	rt_task_set_priority(&low_task, L);
...
void high_task_func(void *arg) {
	...
	print_pri(&high_task, "high : taking mut B\n");
	// rt_sem_v(&semaphore);
	rt_mutex_acquire(&mutB, TM_INFINITE);
	...
	print_pri(&high_task, "high : taking mut A\n");
	// rt_sem_v(&semaphore);
	rt_mutex_acquire(&mutA, TM_INFINITE);
	...
	print_pri(&high_task, "high : return mut A\n");
	// rt_sem_v(&semaphore);
	rt_mutex_release(&mutA);

	print_pri(&high_task, "high : return mut B\n");
	// rt_sem_v(&semaphore);
	rt_mutex_release(&mutB);
...
int main(int argc, char const *argv[])
{
	...
	RT_TASK_INFO curtaskinfo;
 	...
 	rt_mutex_create(&mutA, "mutA");
 	rt_mutex_create(&mutB, "mutB");
